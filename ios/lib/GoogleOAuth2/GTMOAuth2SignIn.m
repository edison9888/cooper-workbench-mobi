...
//NSString *const kOOBString = @"https://cooper.apphb.com/Account/GoogleLogin";
//NSString *const kOOBString = @"https://hi.xiaoxuan.com/Account/GoogleLogin";
//NSString *const kOOBString = @"http://debug.incooper.net/Account/GoogleLogin";
...
@synthesize shouldFetchGoogleUserTasks = shouldFetchGoogleUserTasks_;
@synthesize shouldFetchGoogleUserCalendar = shouldFetchGoogleUserCalendar_;
...
- (BOOL)startSigningIn {
  ...
  if (self.shouldFetchGoogleUserProfile) {
    ...
    if(self.shouldFetchGoogleUserEmail) {
        NSString *const tasksScope = @"https://www.googleapis.com/auth/tasks";
        NSString *scope = auth.scope;
        if ([scope rangeOfString:tasksScope].location == NSNotFound) {
            scope = [GTMOAuth2Authentication scopeWithStrings:scope, tasksScope, nil];
            auth.scope = scope;
        }
    }
    
    if(self.shouldFetchGoogleUserEmail) {
        NSString *const calendarsScope = @"https://www.googleapis.com/auth/calendar";
        NSString *scope = auth.scope;
        if ([scope rangeOfString:calendarsScope].location == NSNotFound) {
            scope = [GTMOAuth2Authentication scopeWithStrings:scope, calendarsScope, nil];
            auth.scope = scope;
        }
    }
#endif
