...
- (void)webViewDidStartLoad:(UIWebView *)webView {
    ...
    //NSLog(@"webView start header: %@", [headers description]);
    
    //NSLog(@"webView start url: %@", r.URL.absoluteString);
...
- (void)requestFinished:(ASIHTTPRequest *)request
{
    ...
    NSString *email = [request.responseString stringByReplacingOccurrencesOfString:@"\"" withString:@""];
    [[Constant instance] setUsername:email];
    [[Constant instance] setLoginType:@"google"];
    [Constant saveToCache];
    
    [self popView];
    [loginDelegate googleLoginFinish:nil];
}
